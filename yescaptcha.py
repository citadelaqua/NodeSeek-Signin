...
class YesCaptchaSolver:
    ...
    def _create_task(
        self, url: str, sitekey: str, user_agent: Optional[str] = None, verbose: bool = False
    ) -> Optional[str]:
        """创建验证码任务并返回任务ID"""
        ...
        try:
            ...
            response = requests.post(self.create_task_url, json=data, timeout=self.timeout, impersonate="chrome110")
    ...
    def _get_task_result(self, task_id: str, verbose: bool = False) -> Optional[str]:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                response = requests.post(self.get_result_url, json=data, timeout=self.timeout, impersonate="chrome110")
                result = response.json()
