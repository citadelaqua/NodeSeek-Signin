...
class TurnstileSolver:
    ...
    def __init__(
        self, api_base_url: str, client_key: str, max_retries: int = 20, retry_interval: int = 6, timeout: int = 60
    ):
        """
    ...
    def solve(
        ...
        payload_dict = {"clientKey": self.client_key, "type": "Turnstile", "url": url, "siteKey": sitekey}
        ...
        try:
            ...
            response = requests.post(self.create_task_url, data=payload, headers=headers, timeout=self.timeout)
            ...
            # 准备获取结果参数
            result_payload_dict = {"clientKey": self.client_key, "taskId": task_id}
            ...
            for attempt in range(1, self.max_retries + 1):
                ...
                result_response = requests.post(
                    self.get_result_url, data=result_payload, headers=headers, timeout=self.timeout
                )
                ...
                if verbose and result_data.get("status") != "completed":
