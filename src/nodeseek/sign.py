...
def delete_ql_env(var_name: str):
    ...
    try:
        ...
        env_result = QLAPI.getEnvs({"searchValue": var_name})  # noqa: F821
        ...
        if env_ids:
            ...
            delete_result = QLAPI.deleteEnvs({"ids": env_ids})  # noqa: F821
...
def save_cookie_to_ql(var_name: str, cookie: str):
    ...
    try:
        ...
        create_result = QLAPI.createEnv(create_data)  # noqa: F821
...
def session_login(user, password, solver_type, api_base_url, client_key):
    try:
        if solver_type.lower() == "yescaptcha":
            ...
            solver = YesCaptchaSolver(api_base_url=api_base_url or "https://api.yescaptcha.com", client_key=client_key)
        else:  # 默认使用 turnstile_solver
            print("正在使用 TurnstileSolver 解决验证码...")
            solver = TurnstileSolver(api_base_url=api_base_url, client_key=client_key)

        token = solver.solve(
            url="https://www.nodeseek.com/signIn.html", sitekey="0x4AAAAAAAaNy7leGjewpVyR", verbose=True
        )
        if not token:
            print("验证码解析失败")
            return None
    ...
    data = {"username": user, "password": password, "token": token, "source": "turnstile"}
    headers = {
        ...
        for record in all_records:
            ...
            if record_time_shanghai >= query_start_time and "签到收益" in description and "鸡腿" in description:
                signin_records.append(
                    {"amount": amount, "date": record_time_shanghai.strftime("%Y-%m-%d"), "description": description}
                )
...
if __name__ == "__main__":
    ...
    if len(accounts) == 0 and len(cookie_list) > 0:
        for _i in range(len(cookie_list)):
