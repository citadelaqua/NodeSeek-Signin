#!/usr/bin/env bash
# scripts/backend/start.sh — 后台启动后端（生产模式，nohup）
set -euo pipefail
...
if [[ -f "$PID_FILE" ]]; then
    ...
    if kill -0 "$OLD_PID" 2>/dev/null; then
        warn "后端已在运行（PID $OLD_PID），跳过启动"
        warn "如需重启请运行: scripts/backend/restart.sh"
        exit 0
    else
...
if [[ -f "$REPO_ROOT/.venv/bin/python" ]]; then
    ...
    err "未找到 Python，请先创建虚拟环境: python3 -m venv .venv && pip install -r requirements-web.txt"
    exit 1
fi
...
if [[ -f "$REPO_ROOT/.env" ]]; then
    ...
    warn ".env 文件不存在，请参考 .env.example 创建"
fi
